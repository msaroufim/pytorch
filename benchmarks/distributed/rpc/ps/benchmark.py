...
class GradientServer:
    ...
    def __init__(self, rank, trainer_count, use_cuda_rpc, batch_mode):
...
def register_hook(hook_id, ddp_model, process_group, gs_rref, world_size, rank, use_cuda_rpc, futures, bp_location, new_batch_indicator, metrics):
    ...
    def send_request(tensor, sparse=True, dense=False):
        ...
        if tensor.is_sparse and sparse:
            record_event(rank, HOOK_METRIC, "{},{}".format(new_batch_indicator[0],bp_location[0]), 
            "sparse_rpc", metrics)
            ...
            fut = rpc.rpc_async(
                ...
                kwargs={
                    ...
                    SIZE: tensor_size})
            record_event(rank, HOOK_METRIC, "{},{}".format(new_batch_indicator[0],bp_location[0]), "sparse_rpc", metrics)
        elif not tensor.is_sparse and dense:
            record_event(rank, HOOK_METRIC, "{},{}".format(new_batch_indicator[0],bp_location[0]), "dense_rpc", metrics)
            fut = rpc.rpc_async(
                gs_rref.owner(),
                GradientServer.average_gradient,
                args=(
    ...
    if hook_id == 1:
        ...
        def rpc_for_sparse_nccl_allreduce_dense_hook(state, bucket):
            ...
            if tensor.is_sparse:
                ...
                record_event(rank, HOOK_METRIC, "{},{}".format(new_batch_indicator[0],bp_location[0]), "nccl_allreduce_dense", metrics)
        ...
        def nccl_all_reduce_hook(state, bucket):
            tensor=bucket.get_tensor()
            tensors_count = len(get_tensors(bucket))
            record_event(rank, HOOK_METRIC, "{},{}".format(new_batch_indicator[0],bp_location[0]), "nccl_allreduce", metrics)
            fut=process_group.allreduce(tensor).get_future()
            record_event(rank, HOOK_METRIC, "{},{}".format(new_batch_indicator[0],bp_location[0]), "nccl_allreduce", metrics)
            bp_location[0] += tensors_count
            return fut
        ...
        def gloo_allreduce_hook(state, bucket):
            tensor = bucket.get_tensor()
            tensors_count = len(get_tensors(bucket))
            record_event(rank, HOOK_METRIC, "{},{}".format(new_batch_indicator[0],bp_location[0]), "gloo_allreduce", metrics)
    ...
    if rank == world_size - 1:
        ...
        gs_rref=rpc.remote(
            ...
            kwargs={
                ...
                BATCH_MODE:configurations.batch_mode
                }
        )
        futs=[
            rpc.rpc_async(
